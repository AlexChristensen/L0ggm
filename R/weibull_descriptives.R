...
#'
#' @format A 223 \eqn{\times} 9 data frame with the following columns:
#' \describe{
...
#'   \item{mean_weight}{Numeric. Mean of the absolute partial correlations in
#'   the lower triangle of the partial correlation matrix.}
#'   \item{snr}{Numeric. Signal-to-noise ratio of the absolute partial
#'   correlations, computed as \code{mean_weight / sd(|partial correlations|)}.}
#'   \item{ope}{Numeric. Observations-per-edge ratio, computed as sample size
#'   divided by the number of unique edges: \eqn{n \,/\, (p(p-1)/2)}. Reflects
#'   the amount of data available per estimated parameter.}
#'   \item{condition}{Numeric. Condition number of the empirical correlation
#'   matrix, computed via \code{\link{kappa}}. Large values indicate
#'   near-multicollinearity among variables, which can destabilize partial
...
#' matrices of the same size) were removed. Networks were further filtered to
#' retain only those with \eqn{8 \le p \le 40} nodes and \eqn{n < 300{,}000}
#' observations.
#'
...
#' remaining edges were negative (\eqn{\bar{x} = 0.351}, \eqn{SD = 0.083}),
#' consistent with mixed-sign partial correlation structures commonly observed
#' in psychological network data.
#'
#' Note that the derived scaling indices used to fit \code{\link{weibull_weights}}
#' (\code{beta_min}, \code{rlp}, \code{n_scaled}) are not stored here; they are
#' computed on the fly from \code{n} and \code{p} inside
#' \code{\link{weibull_parameters}}.
#'
#' @keywords datasets
#'
#' @seealso
#' \code{\link{weibull_weights}} for the SUR model fitted to these data;
#' \code{\link{weibull_parameters}} for the function that uses both objects
#' to predict Weibull parameters for new networks.
#'
#' @references
...
#'
#' # Relationship between scale and observations-per-edge
#' plot(weibull_descriptives$ope, weibull_descriptives$scale,
#'      xlab = "Observations per Edge", ylab = "Scale")
#'
NULL
#----